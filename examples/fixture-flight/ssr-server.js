...
async function main() {
	...
	app.all("/", async function (req, res) {
		const { ReactServerDOMWebpackClient, ReactDOMServer } = shared.serverEntry();
		...
		if (req.accepts("text/html")) {
			try {
				...
				const root = await ReactServerDOMWebpackClient.createFromNodeStream(
					rscResponse,
					await shared.getClientModulesSSRManifest()
				);
				// Render it into HTML by resolving the client components
				res.set("Content-type", "text/html");
				const { pipe } = ReactDOMServer.renderToPipeableStream(root, {
					bootstrapScripts: (await shared.getEntryManifest())["client-entry"].js,
				});
				pipe(res);
			} catch (e) {
